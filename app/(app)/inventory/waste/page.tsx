...
export default function WastePage() {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
      ...
      <KpiGrid>
        <KpiCard label="Waste Entries" value={loading ? "…" : rows.length} icon={Trash2} />
        <KpiCard label="Value Wasted" value={loading ? "…" : formatCurrency(totalValue)} icon={Wallet} />
      </KpiGrid>
      ...
      {error ? (
        ...
        <DataTable columns={columns} data={rows} searchPlaceholder="Search waste log…" emptyMessage="No waste logged yet." />
      )}
    </div>
  );
}
