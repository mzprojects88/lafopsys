...
function Bin({ bin, lots }: { bin: StorageLocationRow; lots: LotOnHandRow[] }) {
  return (
    <div className="flex items-start justify-between gap-3 text-theme-xs">
      <span className="text-muted-foreground">
        {bin.name}
        {bin.code ? <span className="ml-1 font-mono">({bin.code})</span> : null}
      </span>
      <div className="flex flex-wrap justify-end gap-1">
        {lots.length === 0 ? (
          ...
          lots.map((lot) => (
            <Badge key={lot.lot_id} variant="secondary">
  ...
  function renderNode(node: StorageLocationRow): React.ReactNode {
    ...
    return (
      <div key={node.id} className="flex flex-col gap-1">
        <span className="text-theme-sm font-medium text-foreground">{node.name}</span>
        <div className="ml-3 flex flex-col gap-1.5 border-l border-border pl-3">{kids.map(renderNode)}</div>
      </div>
    );
  }
    ...
    <div className="flex flex-1 flex-col gap-6">
      ...
      {error ? (
        ...
        <Accordion type="multiple" defaultValue={sites.map((s) => s.id)} className="flex flex-col gap-2">
          {sites.map((site) => (
            <AccordionItem key={site.id} value={site.id} className="rounded-2xl border border-border bg-card px-5">
