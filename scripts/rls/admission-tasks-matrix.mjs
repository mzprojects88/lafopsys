...
async function main() {
  ...
  await scenario(ids, "each seeded rule carries its English", "social_worker", null,
    q("select count(*)::int as n from ops.orientation_topics where topic like 'Magdasal muna%' and topic_en = 'Pray before eating.'"), value("n", 1));
