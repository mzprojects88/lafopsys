 ...
 * exempt rather than being permanently locked out. Inventory-only roles are
 * exempt too, unless an admin has turned on the "require clock-in" setting
 * (see clockInRequired in use-clock-status.ts).
 */
export function ClockInGate({ children }: { children: React.ReactNode }) {
  ...
  const mustClockIn = mounted && !loading && !!me && clockInRequired && !hasClockedInToday && pathname !== "/staff";
