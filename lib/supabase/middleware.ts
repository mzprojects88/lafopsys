...
// /api/calendar/sync and /api/patients/{house,master}-sheet-sync check a bearer secret themselves: the scheduled job that
// calls it has no session, and a redirect to /login would be silent.
const PUBLIC_PATHS = ["/login", "/impact", "/portal/login", "/api/calendar/sync", "/api/patients/house-sheet-sync", "/api/patients/master-sheet-sync"];
