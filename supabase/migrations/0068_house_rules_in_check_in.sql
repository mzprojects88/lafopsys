-- The house rules, enforced by the database (production test, 2026-09-25).
--
-- Until now the rules step lived only in the browser: the stay was saved
-- first and the rule ticks written after it, so a failed write or a closed
-- tab left a child checked in with no rules recorded. check_in and
-- admit_from_sheet now take p_rules_discussed (refused unless true) and tick
-- the family's rules on the new stay in the same transaction. The bodies are
-- otherwise unchanged from 0051/0057 (taken from the live definitions).
-- A group's shared talk (0065) is still recorded by the app.

drop function ops.admit_from_sheet(uuid, text, date, uuid, jsonb, uuid, text, text, text, date);
drop function ops.check_in(text, date, uuid, uuid, uuid, text, text, text, date, date, text, text, text, boolean);

CREATE FUNCTION ops.check_in(p_unit_id text, p_check_in_at date, p_patient_id uuid DEFAULT NULL::uuid, p_referral_id uuid DEFAULT NULL::uuid, p_carer_id uuid DEFAULT NULL::uuid, p_carer_name text DEFAULT NULL::text, p_carer_relationship text DEFAULT NULL::text, p_carer_mobile text DEFAULT NULL::text, p_expected_checkout_at date DEFAULT NULL::date, p_appt_date date DEFAULT NULL::date, p_appt_time text DEFAULT NULL::text, p_appt_clinic text DEFAULT NULL::text, p_appt_purpose text DEFAULT NULL::text, p_appt_needs_transport boolean DEFAULT false, p_rules_discussed boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
revoke all on function ops.check_in(text, date, uuid, uuid, uuid, text, text, text, date, date, text, text, text, boolean, boolean) from public, anon;
grant execute on function ops.check_in(text, date, uuid, uuid, uuid, text, text, text, date, date, text, text, text, boolean, boolean) to authenticated, service_role;
revoke all on function ops.admit_from_sheet(uuid, text, date, uuid, jsonb, uuid, text, text, text, date, boolean) from public, anon;
grant execute on function ops.admit_from_sheet(uuid, text, date, uuid, jsonb, uuid, text, text, text, date, boolean) to authenticated, service_role;
